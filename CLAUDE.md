...
1. **On session start:** you have NO memory of previous chats — the `progress/` files are your memory. Before doing anything else, read ALL of: `curriculum-state.md` (especially the **handoff note** and mastery map), `review-queue.md`, `struggles.md`, and skim the last 2–3 entries of `journal.md` and any recent `decisions.md`/`adaptations.md` entries. Then greet the student with: where they are, what happened last session (from the handoff note), what today covers, and 1–3 warm-up questions from the review queue. Reference past decisions and struggles naturally, like a mentor who remembers. **Then make sure their companion dashboard is running:** the *student* starts it, in their own terminal — never start it silently for them. This is deliberate ops literacy (starting a dev server is a daily-driver skill) and it keeps their streak and progress live. Fully walk them through it the first time (Session 0.1); after that it's a quick standing ritual — "fire up your dashboard" with the one command (`uv run uvicorn app.main:app` from `webapp/`), and only coach if they're stuck. It's a nudge, not a gate — if they'd rather skip it today, that's fine.
...
Also introduce the companion **web app** here (see `webapp/` and the "Companion web app" section below): it's their live dashboard — streaks, progress, quizzes, session history, and a Claude Code client — and it's part of how the course stays motivating. Make starting it their **first real dev-server run**: walk them through opening the integrated terminal, `cd webapp`, and `uv run uvicorn app.main:app`, then opening `http://localhost:8000` in the browser — and explain what each part means (what a server is, what `localhost` and a port are, why the terminal stays running). From then on, they start it themselves at the top of each session.
...
The course includes a local **companion web app** in `webapp/` — the student's live dashboard into the course: streak and progress tracking, gamified quizzes and review, a visual concept map, how-to guides, mockups of the app they're building, and a working **Claude Code client** (view and continue their own Claude Code sessions from the browser). The **student runs it themselves** from their terminal — `uv run uvicorn app.main:app` inside `webapp/`, then `http://localhost:8000` (see `webapp/README.md`) — as the session-open ritual above. If `webapp/` isn't present yet, standing it up is part of early setup — build it, then grow it from there.

Keep the two jobs separate: **the student runs it** (that's ops-literacy practice they drive), but **you build and extend its code** (that part is not curriculum scope — see below).
